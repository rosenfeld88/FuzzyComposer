...
class GaussianSAM:
    ...
    def train(self, data, adapt_iters, lr, epoch_size, model_save, filename):
        mel = 0
        ch = 1
        melodies = data[mel]
        chords = data[ch]
        ...
        learn_note = tf.nn.softmax(tf.multiply(self.cw, tf.clip_by_value(fuzzy_approx, 0, 127)))
            ...
            for m, melody in enumerate(melodies): #For each song
                ...
                for n in range(start, len(melody)): #For each note in the song
                    ...
                    if note + 2 > 129 or note + 2 < 0:
                        print('OH SHIT')
                    _, run_ent = sess.run([train_op, cross_entropy], feed_dict = {conditioner: previous + 2, actual_note: note + 2})
        ...
        spio.savemat(filename + '_ps_error.mat', mdict = {'error': epoch_ent})
    ...
    def generate(self, chord_prog, primer_notes, model_save):
        ...
        with tf.Session() as sess:
            ...
            for i in range(start, num_notes):
                ...
                melody.append(sess.run(learn_note, feed_dict = {conditioner: previous}) - 2)
