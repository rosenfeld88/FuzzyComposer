...
#GET THE DATA SET FROM STORED FILE
def get_data_set():
    ...
    for run in range(1):
        ...
        for type in range(1): #(len(asams)):
            ...
            for i in range(len(models)):
                print(asam_types[i])
                save_model = bases[type] + '/Saved_Models/' + asams[type][i][model_file]
                errors[type][i] = models[i][model].train(melodies_all[i], adapt_iters, lr, epoch_size, save_model, filename)
